...
{:else if message.type === 'AssistantMessage'}
  <div class="flex justify-start">
    <div class="bg-white border border-gray-200 rounded-lg px-4 py-3 max-w-[70%]">
      <div class="flex items-center gap-2 mb-2">
        <div class="w-6 h-6 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full"></div>
        <span class="text-xs font-medium text-gray-600">{message.model}</span>
      </div>
...
{:else if message.type === 'SystemMessage'}
  <div class="flex justify-center">
    <div class="bg-gray-100 text-gray-700 rounded-lg px-4 py-2 text-sm max-w-[70%]">
      <p>{message.content}</p>
    </div>
  </div>
{:else if message.type === 'Error'}
  <div class="flex justify-center">
    <div class="bg-red-50 border border-red-200 text-red-800 rounded-lg px-4 py-3 max-w-[70%]">
